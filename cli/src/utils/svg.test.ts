...
const mergedDir = join(sampleBase, 'merged');

describe('SVG parser', () => {
  it('parses wall.svg with line or path elements', () => {
    const svg = parseSvgFile(join(mergedDir, 'lv-3', 'wall.svg'));
  ...
  it('parses structure_column.svg with rect elements', () => {
    const svg = parseSvgFile(join(mergedDir, 'lv-3', 'structure_column.svg'));
    expect(svg.elements.length).toBeGreaterThan(0);
  ...
  it('no door.svg exists (doors are CSV-only)', () => {
    expect(existsSync(join(mergedDir, 'lv-3', 'door.svg'))).toBe(false);
  });
});
