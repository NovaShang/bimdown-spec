...
name: bimdown-zh
version: 1.1.0
description: AI与建筑信息的桥梁。像写代码一样读写BIM数据！让智能体自动完成建筑建模、算量审查，或仅仅是为你建一栋专属的数据化房屋！
---

# BimDown 中文智能体技能与规范

> **你的使命 (Your Mission)：** 建立 AI 智能体和建筑数据 (Building Data) 的桥梁。让 AI 可以像读写代码一样理解和创建建筑信息模型 (BIM)。从而让你能执行建筑设计、按图建模、工程量统计、模型审查等工作。当然，仅仅是为你建一个自己的专属房屋模型也是非常有趣且完全支持的！
...
7. **发布 (Publish)**: 运行 `bimdown publish <dir>` 来上传项目并获取在浏览器查看3D模型的分享链接。**安全红线**: 这将会把用户的本地项目数据上传到 BimClaw 远程云服务器。为防止数据泄露，在**每个项目首次执行此发布命令前，你必须显式获得用户的授权同意**。（获取过一次授权的项目可自由重复发布）
...
7. **`bimdown publish <dir> [--expires 7d]`**: 将项目发布至 BimClaw 并返回模型预览分享链接。用于让用户通过3D查看器审查模型。**安全红线**: 将项目数据上传至外部服务器。针对每个项目，首次运行此命令前必须询问用户并获得明确同意。
